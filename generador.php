...
<?php
	...
	$datos=$tra->get_biometrico_dist5_user($u,$RegistrosEmpezar,$RegistrosAMostrar,$inicio,$fin);
...
$pdf->Cell(45,6.2, "FECHA FIN: ".$_GET['fin'],1,0,'A');
		$pdf->Ln();$pdf->Ln();
	$pdf->Cell(5,5.8, '#',1,0,'A');
	$pdf->Cell(42,5.8, 'NOMBRES',1,0,'A');
	$pdf->Cell(18,5.8, 'NIVEL',1,0,'A');
	$pdf->Cell(38,5.8, 'CARGO',1,0,'A');
	$pdf->Cell(12,5.8, 'CI',1,0,'A');
...
$nro = date("d", $fech);
 if($d==1) $d='L';
   ...
   $auxiliar = $d.''.$nro;  
    ...
    $pdf->Cell(4.5,5.8, $auxiliar,1,0,'A'); 
}
	$pdf->SetFont('Arial','B',7);
echo $pdf->Cell(5,5.8, "Dia",1,0,'A');
echo $pdf->Cell(8,5.8, "Faltas",1,0,'A');
echo $pdf->Cell(8,5.8, "Atraso",1,0,'A');
echo $pdf->Cell(10,5.8, "Permiso",1,0,'A');
echo $pdf->Cell(10,5.8, "Sancion",1,0,'A');
echo $pdf->Cell(16,5.8, "B Medicas",1,0,'A');
echo $pdf->Cell(24,5.8, "Comision",1,0,'A');
echo $pdf->Cell(16,5.8, "Vacaciones",1,0,'A');
$pdf->Ln();
?>
 <?php
...
<?php 
	...
	$pdf->Cell(5,5.5,$cont,1,0,'A');
    $pdf->Cell(42,5.5, $texto,1,0,'A');
    $pdf->Cell(18,5.5, $NIVEL,1,0,'A');
	$pdf->Cell(38,5.5, $CARGO,1,0,'A');
	$pdf->Cell(12,5.5, $datos[$i]["CI"],1,0,'A');
...
if(strtotime($d[$k]["HORA"]) > strtotime($pat[0]["salida2"]) - (30*60) && strtotime($d[$k]["HORA"]) < strtotime($pat[0]["salida2"]) + (180*60) && $tard==2 ) { 
		...
		//CIERRA PRUEBA CON NUMEROS
		  ...
		  $pdf->SetFont('Arial','B',5.5);	
		  if($productos[0]== '-'){
			   $pdf->Cell(4.5,5.5,$productos[0],1,0,'A');
			  }
		  else{
   		  $pdf->Cell(4.5,5.5,($dia+$dia2).'-'.($tard+$tard2),1,0,'A');
			  }
}		
?>
<?php
  ...
  $pdf->Cell(5,5.5,$datos[$i]["DIAS_PAGO"],1,0,'C');
  $nuevodia = 30-$datos[$i]["DIAS_PAGO"];
  $pdf->Cell(8,5.5,((($resul+$total_faltas)*($datos[$i]["TIPO_DESCUENTO"]))),1,0,'C');
...
$vaca = implode($V3);
  ...
  echo $pdf->Cell(16,5.5,$baja,1,0,'C');
  echo $pdf->Cell(24,5.5,$comi,1,0,'C');
  echo $pdf->Cell(16,5.5,$vaca,1,0,'C');
...
//______________________________________________________________________________________________________________
		  $pdf->Output("Reporte-Detallado.pdf",'D');
		exit;
